...
def test_payment_transactions_start_pending(account, sample_bytes):
    result = import_credit_card_csv(sample_bytes, account)
    payments = [tx for tx in result.transactions if tx.description == "CAPITAL ONE MOBILE PYMT"]
    assert len(payments) == 2
    assert all(tx.model_category is None for tx in payments)
    assert all(tx.model_confidence == -1 for tx in payments)
